...
func ParseIPRange(opts *Options) (ips []string, err error) {
	if opts.Ips == "" {
		return ips, nil
	}
	defer func() {
		if e := recover(); e != nil {
			ips = nil
			err = fmt.Errorf("invalid ip-range : '%s'", opts.Ips)
		}
	}()

	list, err := iprange.ParseList(opts.Ips)
	if err != nil {
		return nil, err
	}
	rng := list.Expand()
...
func ParsePorts(opts *Options) ([]int, error) {
	ports := make([]int, 0, 10)
	if opts.Ports == "" {
		return DefaultPorts, nil
	}
	ranges := strings.Split(opts.Ports, ",")
